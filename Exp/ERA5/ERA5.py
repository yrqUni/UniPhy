...
class ERA5Dataset(Dataset):
    def __init__(
        ...
        self._max_cache = max(4, int(mmap_cache_size))
    ...
    def _get_single_frame(self, global_idx):
        file_idx = (
            np.searchsorted(self.file_frame_offsets, global_idx, side="right") - 1
        )
    ...
    def _load_frames(self, idx, offsets, use_sequential):
        offsets_are_int = all(abs(offset - round(offset)) < 1e-8 for offset in offsets)

        if use_sequential and offsets_are_int:
            start_global = idx + int(round(offsets[0]))
            file_idx = (
                np.searchsorted(self.file_frame_offsets, start_global, side="right") - 1
            )
