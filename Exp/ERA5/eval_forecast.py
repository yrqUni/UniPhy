...
def parse_args():
    ...
    parser.add_argument(
        "--climatology-dir",
        default=None,
        help="Data directory for computing training-period climatology used in ACC. "
        "When omitted, the evaluation set itself is used (less accurate for ACC).",
    )
    parser.add_argument(
        "--climatology-year-range",
        default=None,
        help="Year range for climatology (e.g. '2000,2016'). "
        "Defaults to eval-year-range when --climatology-dir is supplied without this flag.",
    )
...
def main():
    ...
    sample_count = 0
    clim_source = "test_set"
    if args.climatology_dir:
        if args.climatology_year_range is None:
            print(
                "WARNING: --climatology-dir supplied without --climatology-year-range; "
                "defaulting to --eval-year-range. For correct ACC, pass the training "
                "year range explicitly via --climatology-year-range."
            )
            clim_yr = list(eval_year_range)
        else:
            clim_yr = resolve_eval_year_range(args.climatology_year_range)
        ...
        clim_source = f"climatology_dir={args.climatology_dir} years={clim_yr}"
        for batch_idx, (data, _) in enumerate(clim_loader):
            targets = data[:, cond_steps:].float()
            climatology_sum += targets.sum(dim=0).cpu()
            sample_count += data.shape[0]
    else:
        print(
            "WARNING: --climatology-dir not provided. Computing climatology from the "
            "evaluation set itself. ACC values may be over-optimistic. For publication "
            "results, supply a training-period climatology via --climatology-dir."
        )
    ...
    with torch.no_grad():
        for batch_idx, (data, dt_data) in enumerate(loader):
            ...
            for _ in range(ensemble_size):
                z_context = None if ensemble_size == 1 else True
                z_rollout = None if ensemble_size == 1 else True
            ...
            acc_numerator, acc_pred_energy, acc_target_energy = (
                compute_channel_acc_terms(
                    pred_mean,
                    target,
                    climatology_target,
                    lat_weights,
    ...
    result = {
        ...
        "climatology_source": clim_source,
        ...
        "invalid_metric_counts": format_invalid_metric_counts(invalid_metric_counts),
    }
