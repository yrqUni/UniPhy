...
import logging
import datetime
sys.path.append('/mnt/data/ConvLRU/Model/ConvLRU')
...
class Args:
    def __init__(self):
        # Model parameters
        self.input_size = (720, 1440) # use ModelConvLRU_ERA5 convert 721 to 720
        self.input_ch = 89
        self.use_mhsa = True
        self.emb_ch = 128
        ...
        self.dec_hidden_ch = 0 # if dec_hidden_layers_num=0, dec_hidden_ch will be ignored
        ...
        self.ckpt_dir = './exp_convlru_0/ckpt' 
        self.ckpt_step = 0.25
    ...
    for ep in range(args.epochs):
        ...
        for train_step, data in enumerate(train_dataloader, start=1):
            ...
            if rank == 0 and (train_step % int(len(train_dataloader)*args.ckpt_step) == 0 or train_step == len(train_dataloader)):
                save_ckpt(model, ep, train_step, loss, args)

        dist.barrier()
        del inputs, outputs, preds, loss, train_dataset, train_sampler, train_dataloader
        gc.collect()
        torch.cuda.empty_cache()
        dist.barrier()
        ...
        dist.barrier()
        del inputs, outputs, preds, loss, eval_dataset, eval_sampler, eval_dataloader
        gc.collect()
        torch.cuda.empty_cache()
        dist.barrier()
