...
def analyze_probe_response_timescales(ckpt_path, save_path="probe_response_timescales.pdf"):
    device = get_device()
    model, cfg = load_config_and_model(ckpt_path, device)

    print("Scanning all blocks for probe-response timescales...")
    ...
    print(f"\nProbe-response timescale distribution (all layers):")
    ...
    ax1 = axes[0]
    if sns is None:
        hist_density, hist_bins = np.histogram(
            tau_days_viz,
            bins=60,
            range=(0, 60),
            density=True,
        ...
        ax1.plot(hist_centers, hist_density, color="#2ca02c", linewidth=2.5)
    else:
        sns.kdeplot(
            tau_days_viz,
            fill=True,
    ...
    ax1.set_title(
        "Probe-Response Timescale Distribution",
        fontweight="bold",
        pad=10,
    )
    ax1.set_xlim(0, 65)
    ...
    ax2.set_title(
        "(b) Probe-Response Timescales by Layer",
        fontweight="bold",
        pad=10,
    )
...
if __name__ == "__main__":
    ...
    if ckpt_file is None:
        ...
        analyze_probe_response_timescales(ckpt_file)
