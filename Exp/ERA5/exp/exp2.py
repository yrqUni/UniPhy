...
def extract_block_operator(block, dim, h_p, w_p, device, num_modes=64):
    ...
    with torch.no_grad():
        z_out, _, _ = block.forward_step(probe_complex, h_prev, dt, flux_prev)
...
def plot_spectral_analysis(ckpt_path, save_path="transient_growth.pdf"):
    ...
    print("Scanning all blocks for probe-response spectral properties...")
    all_evals_real = []
    all_evals_imag = []
    all_growth_curves = []
    max_peak_growth = -1.0
    ...
    fig.suptitle(
        "Probe-Response Spectral Proxy Across All Layers",
        fontsize=13,
        fontweight="bold",
        y=1.02,
    )
    ...
    ax1.set_title(
        r"(a) Probe-Response Spectrum (Re $\lambda < 0$)",
        fontsize=10,
        fontweight="normal",
        pad=12,
    )
    ...
    ax2.plot(
        common_t,
        all_growth_curves[best_layer_idx],
        color="#c0392b",
        linewidth=2.2,
        label=f"Max Growth (Layer {best_layer_idx})",
        zorder=4,
    )
    ax2.plot(
        common_t,
    ...
    ax2.set_title(
        "(b) Probe-Response Non-Normal Amplification",
        fontsize=10,
        fontweight="normal",
        pad=12,
    )
