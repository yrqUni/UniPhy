...
def setup_file_logging(args: Args, rank: int) -> None:
    if rank != 0: return
...
def save_ckpt(model: torch.nn.Module, opt: torch.optim.Optimizer, epoch: int, step: int, metric: float, args: Args, scheduler: Optional[Any] = None) -> None:
    ...
    ckpt_path = os.path.join(args.ckpt_dir, f"ep{epoch}_step{step}_crps{metric:.4f}.pth")
            ...
            for train_step, (data, dt) in enumerate(train_loader, start=1):
                ...
                if not is_accum:
                    ...
                    if rank == 0:
                        ...
                        if args.use_wandb and global_step % args.wandb_every == 0:
                            wandb.log({
                                "train/crps": avg_crps,
                                "train/l1": avg_l1,
                                "train/rmse": avg_rmse,
                                ...
                                "perf/sec_per_step": elapsed / args.log_every
                            })
