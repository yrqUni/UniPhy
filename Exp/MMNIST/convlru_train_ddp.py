...
import logging
import datetime
sys.path.append('/data/ConvLRU/Model/ConvLRU')
...
class Args:
    def __init__(self):
        ...
        self.ffn_hidden_layers_num = 3
        self.dec_hidden_ch = 256
    ...
    for ep in range(args.epochs):
        ...
        for step, (inputs, outputs) in enumerate(train_dataloader, start=1):
            ...
            if step % args.eval_step == 0 and step != 0:
                ...
                with torch.no_grad():
                    loss_value = 0
                    eval_dataloader = tqdm(eval_dataloader, desc=f"Eval Epoch {ep+1}/{args.epochs} - Step {step} - Strat") if rank == 0 else eval_dataloader
