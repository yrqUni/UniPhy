...
import logging
import datetime
sys.path.append('/data/ConvLRU/Model/ConvLSTM')
...
class Args:
    def __init__(self):
        ...
        self.input_ch = 1
        self.hidden_dim = [64, 64, 64, self.input_ch] 
        self.kernel_size = (3, 3) 
        self.num_layers = int(len(self.hidden_dim))
        ...
        self.epochs = 2048
        self.log_path = "./exp_convlstm_0/logs"
        self.ckpt_dir = "./exp_convlstm_0/ckpt"
        self.ckpt_step = 1024
        self.eval_step = 1024
...
def run_ddp(rank, world_size, args):
    ...
    if rank == 0:
        setup_logging(args)
    model = ConvLSTM(input_dim=args.input_ch, 
                     hidden_dim=args.hidden_dim, 
                     kernel_size=args.kernel_size, 
                     num_layers=args.num_layers,
                     batch_first=args.batch_first).cuda(rank)
    ...
    for ep in range(args.epochs):
        ...
        for step, (inputs, outputs) in enumerate(train_dataloader, start=1):
            ...
            loss = loss_fn(preds[0], torch.concat((inputs[:,1:,:,:,:], outputs), axis=1))
            loss.backward()
            opt.step()
            scheduler.step()
                ...
                with torch.no_grad():
                    ...
                    for _step, (inputs, outputs) in enumerate(eval_dataloader, start=1):
                        ...
                        for _ in range(args.data_eval_n_frames_output):
                            pred, _ = model(inputs)
                            preds.append(pred[0][:, -1:, :, :, :].detach())
                            inputs = torch.cat([inputs[:, 1:, :, :, :], pred[0][:, -1:, :, :, :].detach()], dim=1)
