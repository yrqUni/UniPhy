...
class Args:
    def __init__(self, config_file):
        ...
        self.eval_pretrain_path = config.get("eval_pretrain_path", 'None')
...
def train_model(args):
    ...
    eval_dataloader = DataLoader(eval_dataset, batch_size=args.eval_batch_size, shuffle=False, num_workers=8, pin_memory=True, prefetch_factor=4)
    ...
    for ep in range(args.EPs):
        ...
        for step, (inputs, targets) in enumerate(tqdm(dataloader)):
            ...
            mse_loss = criterion(sigmoid_pred_outputs, inputs[:, 1:])
            B, L, C, H, W = sigmoid_pred_outputs.size()
            ssim_values = ssim_criterion(sigmoid_pred_outputs.reshape(B*L, C, H, W).expand(B*L, 3, H, W), inputs[:, 1:].reshape(B*L, C, H, W).expand(B*L, 3, H, W))
