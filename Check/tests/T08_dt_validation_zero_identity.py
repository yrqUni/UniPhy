...
def run():
    ...
    if getattr(model, "latent_dynamics", "complex") == "real":
        state_latent = model.blocks[0].spatial_mixer(latent_zero)
        state_ref = state_latent.permute(0, 2, 3, 1).contiguous()
        state_ref = state_ref.reshape_as(h_zero)
        flux_ref = state_ref.reshape(2, model.h_patches, model.w_patches, model.embed_dim)
        flux_ref = flux_ref.mean(dim=(1, 2))
    else:
        state_ref = states[0][0]
        flux_ref = states[0][1]
    err_state = float((h_zero - state_ref).abs().max().item())
    err_flux = float((flux_zero - flux_ref).abs().max().item())
