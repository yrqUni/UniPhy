...
def _get_curriculum_sub_steps(epoch, total_epochs, all_sub_steps):
    ...
    将 all_sub_steps 按升序排列，epoch 越靠后允许的最大 sub_step 越大。
    前 1/3 epoch 只用粗粒度（最小的一半），后续全部解锁。
    """
...
def align_step(
    ...
    max_t_for_step = min(
        max_t, max_rollout_steps // sub_step,
    )
    max_t_for_step = max(1, max_t_for_step)
    t = random.randint(1, max_t_for_step)
    ...
    pred_seq = infer_model.forward_rollout(
        x_ctx, dt_ctx, dt_list, chunk_size=chunk_size,
    )
...
def align(cfg):
    ...
    if rank == 0:
        ...
        dt_minutes = [
            cfg["alignment"]["target_dt"] / s * 60 for s in all_sub_steps
        ]
        logger.info(
            "  dt resolutions (min): "
            + ", ".join(f"{d:.1f}" for d in dt_minutes)
        )
        logger.info("=" * 60)
    ...
    for epoch in range(epochs):
        ...
        unlocked = _get_curriculum_sub_steps(
            epoch, total_epochs, all_sub_steps,
        )
        if rank == 0:
            ...
            if rank == 0:
                ...
                if (batch_idx + 1) % log_every == 0:
                    log_msg = (
                        ...
                        f"dt={metrics['dt_minutes']:.1f}min "
                        f"n={metrics['n_iters']} "
                        f"T={metrics['total_dt']:.0f}h | "
                    ...
                    wandb.log(
                        {
                            ...
                            "align/dt_minutes": metrics["dt_minutes"],
