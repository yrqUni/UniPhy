...
class PeriodicDepthwiseConv2d(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, kernel_size: int = 7, conv_type: str = "conv"):
        ...
        self.point_conv = nn.Conv2d(int(in_channels), int(out_channels), kernel_size=1, bias=True)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x_pad = F.pad(x, (self.pad_sp, self.pad_sp, 0, 0), mode="circular")
        x_pad = F.pad(x_pad, (0, 0, self.pad_sp, self.pad_sp), mode="replicate")
        x_spatial = self.spatial_conv(x_pad)
        return self.point_conv(x_spatial)
...
class EfficientSpatialGating(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        g_gate = self.global_mlp(g)
        
        lat = x.mean(dim=-1)
        lat_gate = self.lat_mlp(lat).unsqueeze(-1)
...
class LearnableStateMap(nn.Module):
    def __init__(self, static_ch: int, emb_ch: int, H: int, W: int, W_freq: int, mode: str, rank: int = 64):
        super().__init__()
        ...
        if self.mode == "spectral":
            self.mapper = nn.Sequential(
                ...
                nn.SiLU(),
                nn.AdaptiveAvgPool2d((self.H, self.W_freq)),
            ...
            self.mapper = nn.Sequential(
                nn.Conv2d(self.static_ch, self.emb_ch, kernel_size=3, padding=1),
                nn.SiLU(),
                nn.AdaptiveAvgPool2d((self.H, self.W)),
                nn.Conv2d(self.emb_ch, self.emb_ch, kernel_size=1),
            )
    ...
    def forward(self, static_feats: torch.Tensor) -> torch.Tensor:
        ...
        if self.mode == "spectral":
            ...
            out = out.view(B, self.emb_ch, self.rank, self.H, self.W_freq)
...
class ParallelPhysicalRecurrentLayer(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.norm = nn.GroupNorm(get_safe_groups(self.emb_ch), self.emb_ch)
        ...
        if bool(learnable_init_state) and int(static_ch) > 0:
            self.init_state = LearnableStateMap(int(static_ch), self.emb_ch, self.H, self.W, self.Wf, self.dynamics_mode, self.rank)
        else:
    ...
    def forward_spectral(
        ...
        x_reshaped = x.view(B, L, self.emb_ch, self.H, self.W).permute(0, 2, 1, 3, 4)
        nu_rate, theta_rate, sigma = self.koopman.compute_params(x_reshaped)
        a_koop = self.koopman.build_a_koop(nu_rate, theta_rate, dt_seq, self.H)
        x_inj = self._build_injection_freq(x_reshaped, dt_seq)

        if self.use_noise and self.training:
             sig = sigma.squeeze(2).permute(0, 1, 4, 2, 3).unsqueeze(-2)
             dt_sqrt = torch.sqrt(dt_seq.view(B, L, 1, 1, 1, 1).clamp_min(1e-6))
             noise = torch.randn_like(x_inj, dtype=torch.complex64)
        ...
        if last_hidden_in is None:
            ...
            if last_hidden_in.dtype.is_complex:
                ...
                h_prev = last_hidden_in.view(B, self.emb_ch, self.H, self.W)
                h0 = torch.fft.rfft2(h_prev.float(), norm="ortho").to(torch.complex64)
                h0 = h0.unsqueeze(1).expand(-1, self.rank, -1, -1, -1)
        ...
        y = y_flat.view(B, L, self.rank, self.emb_ch, self.H, self.Wf)

        h_space = torch.fft.irfft2(y, s=(self.H, self.W), norm="ortho").to(x.dtype)
        h_stack = h_space.permute(0, 3, 1, 4, 5, 2).contiguous()
        h_last = h_stack[:, :, -1].contiguous()

        out = self.proj_out(h_stack).squeeze(-1)
        out = out.permute(0, 1, 4, 2, 3).reshape(B * L, self.emb_ch, self.H, self.W)
    ...
    def forward_advection(
        ...
        flow_raw, forcing_raw = self.estimator(x)
        
        flow = flow_raw.view(B, L, 2, self.H, self.W)
        forcing = forcing_raw.view(B, L, self.emb_ch, self.H, self.W)
        dt_scale = (dt_seq / self.dt_ref).view(B, L, 1, 1, 1).to(x.device, x.dtype)
        
        flow = flow * self.flow_scale * dt_scale
        forcing = forcing * dt_scale

        if self.use_noise and self.training:
            noise = torch.randn_like(forcing) * self.noise_scale * torch.sqrt(dt_scale.clamp_min(1e-6))
        ...
        out = h_seq.view(B * L, self.emb_ch, self.H, self.W)
        out = self.norm(out)
        out = self.gate(out)
    ...
    def forward(
        ...
        dt_seq_in = torch.ones(batch_size, seq_len, device=x.device, dtype=x.dtype) if list_t is None else list_t.to(x.device, x.dtype)
        dt_seq = _match_dt_seq(dt_seq_in, seq_len)

        if self.dynamics_mode == "spectral":
            return self.forward_spectral(x, last_hidden_in, dt_seq, static_feats, batch_size, seq_len)
        else:
            return self.forward_advection(x, last_hidden_in, dt_seq, static_feats, batch_size, seq_len)
...
class GatedConvBlock(nn.Module):
    def __init__(self, channels: int, hidden_size: Tuple[int, int], kernel_size: int = 7, cond_channels: Optional[int] = None, conv_type: str = "conv"):
        super().__init__()
        self.dw_conv = PeriodicDepthwiseConv2d(int(channels), int(channels), kernel_size=int(kernel_size), conv_type=str(conv_type))
        self.norm = nn.GroupNorm(get_safe_groups(int(channels)), int(channels))
        
        self.cond_channels = int(cond_channels) if cond_channels is not None else 0
        if self.cond_channels > 0:
            self.cond_proj = nn.Conv2d(self.cond_channels, int(channels) * 2, kernel_size=1)
        else:
            self.cond_proj = None
            
        self.pw_conv_in = nn.Conv2d(int(channels), int(channels) * 2, kernel_size=1)
        self.pw_conv_out = nn.Conv2d(int(channels), int(channels), kernel_size=1)
...
class FeedForwardNetwork(nn.Module):
    def __init__(self, emb_ch: int, input_shape: Tuple[int, int], ffn_ratio: float = 4.0, static_ch: int = 0, conv_type: str = "conv"):
        super().__init__()
        self.emb_ch = int(emb_ch)
        self.hidden_dim = int(self.emb_ch * float(ffn_ratio))
        self.c_in = nn.Conv2d(self.emb_ch, self.hidden_dim, kernel_size=1)
        cond_ch = self.emb_ch if int(static_ch) > 0 else None
        self.block = GatedConvBlock(self.hidden_dim, input_shape, kernel_size=7, cond_channels=cond_ch, conv_type=conv_type)
        self.c_out = nn.Conv2d(self.hidden_dim, self.emb_ch, kernel_size=1)
        self.act = nn.SiLU()
...
class UniPhyBlock(nn.Module):
    ...
    def forward(
        ...
        x_mid, h_out = self.prl_layer(x, batch_size, seq_len, last_hidden_in, list_t=list_t, static_feats=static_feats)
...
class BottleneckAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        B_L, C, H, W = x.shape
        x_flat = x.permute(0, 2, 3, 1).flatten(0, 2)
        shortcut = x_flat
        x_norm = self.norm(x_flat)
        N, Cc = x_norm.shape
        qkv = self.qkv(x_norm).reshape(N, 3, self.num_heads, Cc // self.num_heads).permute(1, 0, 2, 3)
        q, k, v = qkv[0], qkv[1], qkv[2]
        x_attn = F.scaled_dot_product_attention(q, k, v, dropout_p=self.attn_drop.p if self.training else 0.0, scale=self.scale)
        x_attn = x_attn.reshape(N, Cc)
        x_attn = self.proj(x_attn)
        x_attn = self.proj_drop(x_attn)
        out = shortcut + x_attn
        out = out.view(B_L, H, W, C).permute(0, 3, 1, 2).contiguous()
        return out
...
class UniPhyBackbone(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.use_unet = str(arch_mode).lower() != "no_unet"
        ...
        if not self.use_unet:
            ...
            for i in range(layers):
                ...
                if i < layers - 1:
                    ...
                    curr_h = (curr_h + 1) // 2 if curr_h % 2 != 0 else curr_h // 2
                    curr_w = (curr_w + 1) // 2 if curr_w % 2 != 0 else curr_w // 2
            ...
            self.mid_spectral = LowFreqSpectralMixer(C, modes_h=spectral_modes_h, modes_w=spectral_modes_w)
            ...
            self.fusion = nn.Conv2d(C * 2, C, kernel_size=1)
    ...
    def forward(
        ...
        if not self.use_unet:
            ...
            for idx, blk in enumerate(self.uniphy_blocks):
                ...
                x, h_out = blk(x, batch_size, seq_len, h_in, list_t=list_t, cond=cond, static_feats=static_feats)
        ...
        cond_expanded = cond
        if cond_expanded is not None and cond_expanded.size(0) != x.size(0):
            cond_expanded = cond_expanded.repeat(seq_len, 1, 1, 1)

        for i, blk in enumerate(self.down_blocks):
            curr_cond = cond_expanded
            ...
            x, h_out = blk(x, batch_size, seq_len, hs_in_down[i], list_t=list_t, cond=curr_cond, static_feats=static_feats)
            last_hidden_outs.append(h_out)
        ...
        for i, blk in enumerate(self.up_blocks):
            ...
            x, h_out = blk(x, batch_size, seq_len, hs_in_up[i], list_t=list_t, cond=curr_cond, static_feats=static_feats)
            last_hidden_outs.append(h_out)
...
class FeatureEmbedding(nn.Module):
    ...
    def forward(self, x: torch.Tensor, static_feats: Optional[torch.Tensor] = None) -> Tuple[torch.Tensor, Optional[torch.Tensor]]:
        ...
        if cond_expanded is not None:
            cond_expanded = cond_expanded.repeat(L, 1, 1, 1)

        for layer in self.c_hidden:
            x = layer(x, cond=cond_expanded)
...
class ProbabilisticDecoder(nn.Module):
    def __init__(self, out_ch: int, emb_ch: int, dist_mode: str = "gaussian", hidden_factor: Tuple[int, int] = (2, 2)):
        ...
        self.pixel_shuffle = nn.PixelShuffle(self.rh) 
        self.final_out_ch = self.output_ch * 2
        if self.dist_mode == "mdn":
            self.final_out_ch = self.output_ch * 3 * 3
...
class UniPhy(nn.Module):
    ...
    def forward(
        ...
        list_t: Optional[torch.Tensor] = None,
        list_t_future: Optional[torch.Tensor] = None,
        static_feats: Optional[torch.Tensor] = None,
        ...
        if mode == "p":
            ...
            x_emb, cond = self.embedding(x_norm, static_feats=static_feats)
            x_hid, last_hidden_outs = self.uniphy_model(x_emb, B, L, list_t=list_t, cond=cond, static_feats=static_feats)
            out = self.decoder(x_hid, cond=cond)
            
            out_tensor = out.view(B, L, -1, H, W).permute(0, 2, 1, 3, 4).contiguous()
            
            if str(self.decoder.dist_mode).lower() == "gaussian":
                mu, sigma = torch.chunk(out_tensor, 2, dim=1)
                if mu.size(1) == self.revin.num_features:
                    mu = self.revin(mu, "denorm", stats=stats)
                    sigma = sigma * stats.stdev
                return torch.cat([mu, sigma], dim=1), last_hidden_outs
            if out_tensor.size(1) == self.revin.num_features:
        ...
        list_t0 = torch.ones(B, L, device=x.device, dtype=x.dtype) if list_t is None else list_t
        stats = revin_stats if revin_stats is not None else self.revin.stats(x)
        out_list: List[torch.Tensor] = []
        
        x_norm = self.revin(x, "norm", stats=stats)
        x_emb, cond = self.embedding(x_norm, static_feats=static_feats)
        x_hidden, last_hidden_outs = self.uniphy_model(x_emb, B, L, list_t=list_t0, cond=cond, static_feats=static_feats)
        
        x_dec0 = self.decoder(x_hidden, cond=cond)
        x_step_dist = x_dec0.view(B, L, -1, H, W).permute(0, 2, 1, 3, 4)
        x_step_dist = x_step_dist[:, :, -1:, :, :]

        dist_mode = str(self.decoder.dist_mode).lower()
        if dist_mode in ["gaussian", "laplace"]:
            out_ch = int(getattr(self.args, "out_ch", x_step_dist.size(1) // 2))
            ...
            if mu.size(1) == self.revin.num_features:
                ...
                mu_denorm = mu
                scale_denorm = scale
            out_list.append(torch.cat([mu_denorm, scale_denorm], dim=1))
            x_step_mean = mu_denorm
        else:
            out_list.append(x_step_dist)
            x_step_mean = x_step_dist[:, : self.revin.num_features] if x_step_dist.size(1) >= self.revin.num_features else x_step_dist

        future = torch.ones(B, int(out_gen_num) - 1, device=x.device, dtype=x.dtype) if list_t_future is None else list_t_future
        ...
        for t in range(int(out_gen_num) - 1):
            ...
            if curr_x.size(1) == self.revin.num_features:
                x_step_norm = self.revin(curr_x, "norm", stats=stats)
            else:
                x_step_norm = curr_x

            x_in, _ = self.embedding(x_step_norm, static_feats=static_feats)
            x_hidden, last_hidden_outs = self.uniphy_model(x_in, B, 1, last_hidden_ins=last_hidden_outs, list_t=dt, cond=cond, static_feats=static_feats)
            x_dec = self.decoder(x_hidden, cond=cond)
            x_step_dist = x_dec.view(B, 1, -1, H, W).permute(0, 2, 1, 3, 4)
        ...
        return torch.cat(out_list, dim=2)
