...
class UniPhyEncoder(nn.Module):
    def __init__(self, in_ch, embed_dim, patch_size, img_height, img_width):
        ...
        if isinstance(patch_size, (tuple, list)):
            self.ph, self.pw = int(patch_size[0]), int(patch_size[1])
        else:
    ...
    def forward(self, x):
        ...
        z = self.proj(x)
        z = z.reshape(B, T, self.embed_dim, self.h_patches, self.w_patches)
        z = z + self.pos
...
class UniPhyEnsembleDecoder(nn.Module):
    ...
    def forward(self, z, member_idx=None):
        B, T, D, Hp, Wp = z.shape
        z_real = torch.cat([z.real, z.imag], dim=2).reshape(B * T, D * 2, Hp, Wp)

        if member_idx is None:
            member_idx = torch.zeros(B, device=z.device, dtype=torch.long)
        if member_idx.ndim == 0:
            member_idx = member_idx.expand(B)
        if member_idx.shape[0] != B:
            member_idx = member_idx.reshape(-1)[:B]

        emb = self.member_emb(member_idx).reshape(B, 1, D, 1, 1).expand(B, T, D, Hp, Wp)
        emb = torch.cat([emb, torch.zeros_like(emb)], dim=2).reshape(B * T, D * 2, Hp, Wp)

        y = self.proj(z_real + emb)
        y = F.interpolate(y, size=(self.img_height, self.img_width), mode="bilinear", align_corners=False)
        return y.reshape(B, T, self.out_ch, self.img_height, self.img_width)
