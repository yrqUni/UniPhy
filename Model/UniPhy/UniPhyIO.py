...
class UniPhyRealEncoder(UniPhyEncoder):
    def _encode_4d(self, x):
        x = self.padder(x)
        x = self.proj(x)
        x = self.stem(x)
...
class UniPhyRealDecoder(UniPhyEnsembleDecoder):
    def _decode_4d(self, z):
        if z.is_complex():
            z_re = z.real
            z_im = z.imag
            ...
            z_re = z
            z_im = torch.zeros_like(z)
        hidden = self.latent_proj(torch.cat([z_re, z_im], dim=1))
        hidden = hidden + self.block1(hidden)
        hidden = hidden + self.block2(hidden)
        out = self.stage1(hidden)
        out = self.stage2(out)
