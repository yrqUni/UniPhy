...
def pscan_kernel(
    ...
    xr_v = tl.load(x_base + idx_l[:, None] * stride_xl + offs_r[None, :], mask=mask_l[:, None], other=0.0)
    xi_v = tl.load(x_base + idx_l[:, None] * stride_xl + (offs_r[None, :] + R), mask=mask_l[:, None], other=0.0)
    xr_m = tl.reshape(tl.zeros([BLOCK_SIZE, R * R], dtype=tl.float32), (BLOCK_SIZE, R, R))
    xi_m = tl.reshape(tl.zeros([BLOCK_SIZE, R * R], dtype=tl.float32), (BLOCK_SIZE, R, R))
    xr_m = tl.where(offs_r[None, :, None] == 0, xr_v[:, :, None], xr_m)
    xi_m = tl.where(offs_r[None, :, None] == 0, xi_v[:, :, None], xi_m)
    xr, xi = tl.ravel(xr_m), tl.ravel(xi_m)
    if REVERSE:
        eye = (offs_mat // R) == (offs_mat % R)
        ar = tl.where(offs_l[:, None] == 0, tl.where(eye, 1.0, 0.0), ar)
        ai = tl.where(offs_l[:, None] == 0, 0.0, ai)
    _, _, rxr, rxi = tl.associative_scan((ar, ai, xr, xi), axis=0, combine_fn=lambda ar, ai, xr, xi, br, bi, yr, yi: combine_fn(ar, ai, xr, xi, br, bi, yr, yi, R))
...
class _PScanFunction(torch.autograd.Function):
    ...
    def forward(ctx, A, X):
        ...
        pscan_kernel[(B, C)](torch.view_as_real(A), torch.view_as_real(X), torch.view_as_real(Y), A.stride(0), A.stride(1), A.stride(2), X.stride(0), X.stride(1), X.stride(2), L, R, triton.next_power_of_2(L), False)
    ...
    def backward(ctx, grad_output):
        ...
        pscan_kernel[(B, C)](torch.view_as_real(AP), torch.view_as_real(grad_output), torch.view_as_real(dX), AP.stride(0), AP.stride(1), AP.stride(2), grad_output.stride(0), grad_output.stride(1), grad_output.stride(2), L, R, triton.next_power_of_2(L), True)
