...
def mat2x2_pscan_kernel(
    ...
    tl.store(
        Y_base + read_offs * stride_x_time + stride_x_d2 + 1,
        y01i, mask=mask,
    )
    tl.store(
        Y_base + read_offs * stride_x_time + stride_x_d1 + 0,
        y10r, mask=mask,
    )
    tl.store(
        Y_base + read_offs * stride_x_time + stride_x_d1 + 1,
        y10i, mask=mask,
    )
    tl.store(
...
class _PScanFunction(torch.autograd.Function):
    ...
    def backward(ctx, grad_Y):
        ...
        grad_X = (
            grad_X_full[..., :D2_orig] if ctx.pad_d2 else grad_X_full
        )
