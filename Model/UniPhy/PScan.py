...
def run_mat2x2_pscan(A_real, X_real, L, reverse=False):
    if HAS_TRITON and A_real.is_cuda:
    ...
    A_complex = torch.view_as_complex(A_real.contiguous())
    X_complex = torch.view_as_complex(X_real.contiguous())
    Y_complex = run_mat2x2_pscan_torch(A_complex, X_complex, reverse)
    return torch.view_as_real(Y_complex).contiguous()
