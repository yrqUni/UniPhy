...
class StochasticHamiltonianPropagator(nn.Module):
    ...
    def forward(self, z, dt):
        if TRITON_AVAILABLE and z.is_cuda:
            ...
            noise_real = torch.randn_like(z_spec.real)
            noise_imag = torch.randn_like(z_spec.imag)
            noise = torch.complex(noise_real, noise_imag)
            
            sigma = F.softplus(self.noise_scale)
            diffusion = sigma * math.sqrt(dt) * noise
            
            z_shifted_spec = z_spec * propagator + diffusion
            z_shifted = torch.fft.ifft2(z_shifted_spec, s=(self.freq_h, self.freq_w), norm='ortho')
            return z_shifted
