...
def check_basis_invertibility():
    ...
    if err < 1e-12:
        ...
        print(f"Basis Inversion Error: {err:.2e}")
...
def check_eigenvalue_stability():
    ...
    if max_real <= 1e-6:
        ...
        print(f"Eigenvalue Stability Error: Max Real Part {max_real:.2e} > 0")
...
def check_ffn_causality():
    ...
    if diff < 1e-12:
        ...
        print(f"FFN Causality Error: {diff:.2e}")
...
def check_full_model_consistency():
    ...
    with torch.no_grad():
        ...
        for block in model.blocks:
            h_state = torch.zeros(
                B_z * H_z * W_z, 1, block.dim, dtype=torch.cdouble, device=device
            )
            ...
            for t in range(T):
                ...
                z_next, h_next, flux_next = block.forward_step(
                    x_step, h_state, dt_step, flux_state
                )
