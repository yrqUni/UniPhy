...
def check_pscan_compatibility():
    ...
    X_diag = torch.randn(B, T, C, D, dtype=torch.complex64, device=device)

    Y_diag = pscan(A_diag, X_diag)

    Y_seq = torch.zeros_like(X_diag)
    Y_seq[:, 0] = X_diag[:, 0]
    for t in range(1, T):
        Y_seq[:, t] = A_diag[:, t] * Y_seq[:, t - 1] + X_diag[:, t]

    diff_diag = (Y_diag - Y_seq).abs().max().item()

    print(f"Diagonal Mode Shape: A={A_diag.shape}, X={X_diag.shape}")
