...
from UniPhyOps import UniPhyPropagator, GlobalFluxTracker, PhysicalBasis
...
def check_basis_invertibility():
    ...
    dim = 64
    basis = PhysicalBasis(dim).to(device)
...
def check_gradient_flow():
    ...
    print(f"Max Gradient: {max_grad:.2e} ({max_grad_name})")
    print(f"Min Gradient: {min_grad:.2e} ({min_grad_name})")

    passed = x.grad is not None and params_with_grad > 0
    print(f"Test Passed: {passed}")
    print()
    return passed
