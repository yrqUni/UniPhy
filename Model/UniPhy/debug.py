...
def test_temporal_propagator(device):
    ...
    # Flux Logic
    flux_A, flux_X = prop.flux_tracker.get_operators(x_eigen) # Using x_eigen as dummy mean
    h = torch.zeros(B, D, device=device, dtype=torch.complex128)
    h_list = []
    for t in range(T):
        h = flux_A[:, :, t] * h + flux_X[:, :, t]
    ...
    u_t = forcing_term * op_forcing.unsqueeze(-1)
    
    # Main Propagator Scan (Manual)
    A_main = op_decay.unsqueeze(-1).expand(B, T, D).permute(0, 2, 1) # (B, D, T)
    X_main = u_t.permute(0, 2, 1) # (B, D, T)
    ...
    h_eigen = torch.stack(h_main_list, dim=2).permute(0, 2, 1) # (B, T, D)
    ...
    for t in range(T):
        x_t = x_input[:, t].unsqueeze(1) # (B, 1, D)
        dt_t = dt[:, t]
        
        x_t_encoded = prop.basis.encode(x_t).squeeze(1) # (B, D)
