...
def sequential_pscan_diag(A, X):
    ...
    for t in range(1, L):
        Y_t = (
            torch.einsum("bcij,bcjk->bcik", A_mat[:, t], Y_list[t - 1])
            + X[:, t]
        )
        Y_list.append(Y_t)
...
def check_backward_diag():
    ...
    A_data = (
        torch.randn(B, L, C, D, dtype=torch.complex64, device="cuda") * 0.5
    )
...
def check_backward_mat():
    ...
    A_data = (
        torch.randn(B, L, C, D, D, dtype=torch.complex64, device="cuda") * 0.3
    )
    X_data = torch.randn(B, L, C, D, 1, dtype=torch.complex64, device="cuda")
...
def check_various_shapes():
    ...
    for B, L, C, D in shapes:
        ...
        A_mat = (
            torch.randn(B, L, C, D, D, dtype=torch.complex64, device="cuda")
            * 0.3
        )
        X = torch.randn(B, L, C, D, 1, dtype=torch.complex64, device="cuda")
