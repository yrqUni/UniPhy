...
class MassConservingSwiGLU(nn.Module):
    def __init__(self, dim, hidden_dim):
        ...
        self.heat_distributor = nn.Parameter(torch.randn(1, dim, 1, 1) * 0.02)
        self.heat_gate = nn.Parameter(torch.zeros(1))
    ...
    def forward(self, x, heat_gain=None):
        ...
        if heat_gain is not None:
            dist_weights = F.softmax(self.heat_distributor, dim=1)
            gate_scale = torch.sigmoid(self.heat_gate)
            
            heat_update = heat_gain * dist_weights * gate_scale
            out = out + heat_update
...
class ThermodynamicVectorMLP(nn.Module):
    ...
    def forward(self, x):
        ...
        magnitudes = torch.sqrt(ke_in + 1e-6)
...
class LieAlgebraRotation(nn.Module):
    def __init__(self, dim):
        super().__init__()
        self.dim = dim
        self.skew_generator = nn.Parameter(torch.randn(dim, dim) * 0.001)
