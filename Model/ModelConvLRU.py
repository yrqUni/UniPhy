...
class Decoder(nn.Module):
    def __init__(self, args, input_downsp_shape):
        ...
        self.c_attn_in = nn.Conv2d(self.emb_ch, args.dec_attn_ch, kernel_size=3, padding='same')
        attn_dim = args.dec_attn_ch*input_downsp_shape[1]*input_downsp_shape[2]
        self.remember_mixer = Transformer(args.dec_attn_layers_num, 
                                          attn_dim,
                                          args.dec_attn_num_heads, 
                                          attn_dim*args.dec_attn_ffn_dim_factor, 
                                          args.dec_attn_dropout)
        self.c_attn_out = nn.Conv2d(args.dec_attn_ch, self.emb_ch, kernel_size=3, padding='same')
